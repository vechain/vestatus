...
function buildRow(network: string, b: ExpandedBlock): IPoint {
    ...
    return {
        ...
        fields: {
            ...
            tx_count: txs.length,
            clause_count: txs.reduce((n, tx) => n + tx.clauses.length, 0),
            paid: txs.reduce((n, tx) => n + parseInt(tx.paid), 0),
            reward: txs.reduce((n, tx) => n + parseInt(tx.reward), 0),
            new_contract_count: txs.reduce((n, tx) => tx.outputs.reduce((n, o) => n + (o.contractAddress ? 1 : 0), n), 0),
            delegated_tx_count: txs.reduce((n, tx) => n + (tx.delegator ? 1 : 0), 0),
